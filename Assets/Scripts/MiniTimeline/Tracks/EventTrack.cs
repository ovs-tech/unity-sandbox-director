...
{
    /// <summary>
    /// Event track for triggering signals and markers
    /// Fires events when playhead crosses signal markers
    /// </summary>
    public class EventTrack : MiniTrackBase<SignalClip>
    {
        public override int Order => 0; // Event tracks run first to allow other systems to react
        ...
        protected override void OnPrepare()
        {
            ...
            Debug.Log($"[EventTrack] Prepared track '{Id}' with {clips.Count} signal clips");
        }
        ...
        protected override void OnCleanup()
        {
            ...
            Debug.Log($"[EventTrack] Cleaned up track '{Id}'");
        }
        ...
        private void FireEvent(SignalClip signal, float currentTime, bool scrub, float previousTime)
        {
            ...
            try
            {
                ...
                Debug.Log($"[EventTrack] Fired event '{signal.eventId}' at time {signal.Start:F2} (payload: '{signal.payload}')");
            }
            catch (Exception e)
            {
                Debug.LogError($"[EventTrack] Error firing event '{signal.eventId}': {e.Message}");
            }
        }
        ...
        public void RegisterEventHandler(string eventId, Action<TimelineEvent> handler)
        {
            ...
            Debug.Log($"[EventTrack] Registered handler for event '{eventId}'");
        }
