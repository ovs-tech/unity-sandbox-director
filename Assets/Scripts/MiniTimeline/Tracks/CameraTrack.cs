...
namespace MiniTimeline.Tracks
{
    ...
    /// </summary>
    public class CameraTrack : MiniTrackBase<CameraClip>
    {
        ...
        private CameraClip nextClip;
        private float blendWeight = 0f;
        
        // Performance optimization
        private readonly List<CameraClip> tempActiveClips = new List<CameraClip>();
        
        #region Track Lifecycle
        
        protected override void OnPrepare()
        {
            Debug.Log($"[CameraTrack] OnPrepare called for track '{Id}', target object: {targetObject}");
            
            if(targetObject is Camera camera)
            {
                targetCamera = camera;
                cameraTransform = camera.transform;
                Debug.Log($"[CameraTrack] Target is Camera: {camera.name}");
            }
            else if (targetObject is GameObject go)
            {
                ...
                if (targetCamera != null)
                {
                    ...
                    Debug.Log($"[CameraTrack] Target is GameObject: {go.name}, Camera found: {targetCamera != null}");
                }
            }
            else
            {
                Debug.LogError($"[CameraTrack] Target object for track '{Id}' is not a Camera or GameObject with Camera");
                return;
            }
            
            if (targetCamera == null)
            {
                Debug.LogError($"[CameraTrack] No Camera found for track '{Id}'");
                return;
            }
            
            Debug.Log($"[CameraTrack] Prepared track '{Id}' with {clips.Count} clips");
        }
        ...
        protected override void OnEvaluate(float time, bool scrub)
        {
            ...
            {
                Debug.LogWarning($"[CameraTrack] Camera is null during evaluation");
                return;
            }
            ...
            Debug.Log($"[CameraTrack] Evaluating at time {time:F2}, found {tempActiveClips.Count} active clips");
        ...
        protected override void OnCleanup()
        {
            currentClip = null;
            nextClip = null;
            Debug.Log($"[CameraTrack] Cleaned up track '{Id}'");
        }
        ...
        /// </summary>
        private void ApplySingleClip(CameraClip clip, float time)
        {
            Debug.Log($"[CameraTrack] Applying single clip '{clip.Id}' at time {time:F2}");
        ...
        /// </summary>
        private void BlendMultipleClips(List<CameraClip> activeClips, float time)
        {
            Debug.Log($"[CameraTrack] Blending {activeClips.Count} clips at time {time:F2}");
        ...
        /// </summary>
        private void ApplyCameraProperties(CameraClip clip, float normalizedTime, float weight)
        {
        ...
        /// </summary>
        private Vector3 EvaluatePosition(CameraClip clip, float normalizedTime)
        {
        ...
        /// </summary>
        private Quaternion EvaluateRotation(CameraClip clip, float normalizedTime)
        {
            if (clip.animationCurve == CameraAnimationCurve.Linear)
            {
        ...
        /// </summary>
        private float EvaluateFieldOfView(CameraClip clip, float normalizedTime)
        {
            if (clip.animationCurve == CameraAnimationCurve.Linear)
            {
        ...
        public CameraClip AddPositionClip(float start, float duration, Vector3 startPos, Vector3 endPos)
        {
        ...
        /// <returns>Created clip</returns>
        public CameraClip AddRotationClip(float start, float duration, Quaternion startRot, Quaternion endRot)
        {
            var clip = new CameraClip
            {
                Id = Guid.NewGuid().ToString(),
        ...
        /// <returns>Created clip</returns>
        public CameraClip AddFieldOfViewClip(float start, float duration, float startFOV, float endFOV)
        {
            var clip = new CameraClip
            {
                Id = Guid.NewGuid().ToString(),
