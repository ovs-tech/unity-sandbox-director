...
namespace MiniTimeline.UI.MVVM.Timeline
{
    ...
    public class TimelineEditorModel
    {
        ...
        // Director reference (non-serialized)
        ...
        {
            // Defer singleton access to avoid serialization errors
            // SubscribeToCommandManager will be called from Initialize
        }
        ...
        private void SubscribeToCommandManager()
        {
            ...
            commandManager.OnRedoPerformed += (cmd) => OnRedoPerformed?.Invoke();
            commandManager.OnStacksChanged += (canUndo, canRedo) =>
            {
                OnCommandStacksChanged?.Invoke(canUndo, canRedo);
            };
        }
        ...
        public void Dispose()
        {
            ...
            if (commandManager != null)
            {
                ...
                commandManager.OnUndoPerformed -= (cmd) => OnUndoPerformed?.Invoke();
                commandManager.OnRedoPerformed -= (cmd) => OnRedoPerformed?.Invoke();
                commandManager.OnStacksChanged -= (canUndo, canRedo) => OnCommandStacksChanged?.Invoke(canUndo, canRedo);
            }
        }
